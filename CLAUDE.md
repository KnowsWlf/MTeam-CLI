...
- `digest.py` — 高分新片摘要：复用 `search_torrents` 拉 movie/tvshow，本地按 IMDB 阈值 + 发布时间窗过滤排序。`fetch_high_score_digest` + `format_digest`。供 `digest` 命令与签到 runner 共用。**digest 每账户隔离**：每个开了 `MTEAM_DIGEST_ENABLED_i` 的账户用自己的 api_key + 自己解析出的 `DigestConfig` 独立拉取（不再全站共享一次）。配置三级优先级：命令行 > 账户覆盖 `MTEAM_DIGEST_*_i` > 全局默认 `MTEAM_DIGEST_*`，合并规则收敛在 `Account.resolved_digest_config(settings)`（唯一来源）。`_coalesce`（非 `or`）保 `min_imdb=0.0`/`limit=0` 不被吞。digest 拉取失败只记日志、签到照常——**digest 永不影响保活**。
